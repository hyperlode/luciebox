...
void timeSecondsToClockString(char * textBuf, unsigned int seconds){
	...
	if (seconds <= 3599){ //3599  for one hour 5999 for 90 minutes
		intToDigitsString(textBuf, (seconds/60)*100 + seconds % 60, true);	
	}else if (seconds <= 35999){ //9h59 (and 59 seconds)
   
		intToDigitsString(textBuf, (seconds/ 3600)*1000 + seconds % 3600 /60, true);	
		textBuf[1] = 'H';
	}else{ //>35999   ==> max uuHm  bvb. 99u9
...
void timeMillisToClockString(char * textBuf, long milliSeconds){  //, bool countDownTimer
