...
void Apps::modeSequencer()
{
	...
	if (this->app_init_edge)
	{
		SEQUENCER_SPEED.start(-200);
		SEQUENCER_EEPROM_MODE_BLINK.start(-1000);

		// load autogenerated song
		uint8_t base_note = random(77,87);
		for (uint8_t i = 0; i < 32; i++)
		{

			// base sequence for all four bars equal
			if (i<8){
				if(!random(0,2)){
			...
			// extra notes to add some melody, it's ok to overwrite base sequence sometimes.
			if(!random(0,4)){
